...
//checking longest number
    bool check_head1(Node* head1,Node* head2)
    {
        ...
        while(head1!=NULL && head2!=NULL) head1=head1->next,head2=head2->next;
        if(head1) return true;
        ...
        if(t1!=NULL && t2!=NULL && t1->data > t2->data) return true;
        return false;
    }
...
//main code
    Node* subLinkedList(Node* head1, Node* head2) {
        ...
        head1=check_0(head1);
        head2=check_0(head2);
        if(!check_head1(head1,head2)) swap(head1,head2);
        head2=rev(head2);
        head1=rev(head1);
        Node *a=new Node(0);
        if(head1==NULL && head2==NULL) return a;
        Node *nn=new Node(-1);
        Node *res=nn;
        ...
        if(head1) nn->next=head1;
        if(head2) nn->next=head2;
